            ...
            try
            {
                ...
                using (StreamReader sr = new StreamReader(fs))
                {
                    while ((Line = sr.ReadLine()) != null)
                    {
                        ...
                        //You melee
                        mc = Regex.Matches(Line, "you (crush|hit|punch|kick|slash|bite|bash|pierce|strike) (.+) for (\\d+) points of");
                        if (mc.Count > 0)
                        {
                            var vDamage = Convert.ToInt32(mc[0].Groups[3].Value);
                            var vDmgModel = new DmgModel()
                        ...
                        //You Miss
                        mc = Regex.Matches(Line, "you try to (crush|hit|punch|kick|slash|bite|bash|pierce|strike) (.+), but miss!");
                        if (mc.Count > 0)
                        {
                            var vDmgModel = new DmgModel()
                            {
